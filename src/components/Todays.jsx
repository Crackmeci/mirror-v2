...
function Archive() {
  ...
  useEffect(()=>{
    document.title = `${import.meta.env.VITE_SITE_NAME} - Today's Zones`
  },[])

  useEffect(()=>{
    if(currentPage != 1){navigate(`/todays`)}
    setTemp(temp+1)
  },[limit])
  ...
  useEffect(()=>{
    setLoading(true);
    Moment.locale('tr');
    axios.get(`${import.meta.env.VITE_APP_URL}/todays/${currentPage}/${limit}`)
  ...
  useEffect(()=>{
    ...
    .then(async res => { await setCount(Math.ceil(res.data["todayZone_count"] / limit)) })
    .catch(err => { console.log(err);})
    .finally(()=>{setLoading(false) })
  },[zones])
  return (
    <div className="w-full mt-4 rounded-[8px] p-2 bg-[#0e1726]">
      <div className='p-4'>
        ...
        <Pagination setTemp={setTemp} temp={temp} refer={`/todays`} page={count} active={currentPage}/>
        </>
