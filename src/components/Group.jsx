...
function Group() {
  ...
  useEffect(()=>{
    document.title = `${import.meta.env.VITE_SITE_NAME} - ${group}'s zones - ${currentPage}`
  },[])

  useEffect(()=>{
    if(currentPage != 1){navigate(`/group/${group}`);}
    setTemp(temp+1)
  },[limit])
  ...
  useEffect(()=>{
    setLoading(true);
    Moment.locale('tr');
    axios.get(`${import.meta.env.VITE_APP_URL}/group/${group}/${currentPage}/${limit}`)
  ...
  useEffect(()=>{
    setLoading(true);
    axios.get(`${import.meta.env.VITE_APP_URL}/groupZoneCount/${group}`)
  ...
  return (
    <div className="w-full mt-4 rounded-[8px] p-2 bg-[#0e1726]">
      <div className='p-4'>
        ...
        <Pagination setTemp={setTemp} temp={temp} refer={`/group/${group}`} page={count} active={currentPage}/>
        </>
