...
function Hacker() {
  ...
  useEffect(()=>{
    document.title = `${import.meta.env.VITE_SITE_NAME} - ${nick}'s zones - ${currentPage}`
  },[])


  useEffect(()=>{
    if(currentPage != 1){navigate(`/hacker/${nick}`);}
    setTemp(temp+1)
  },[limit])
  ...
  useEffect(()=>{
    setLoading(true);
    Moment.locale('tr');
    axios.get(`${import.meta.env.VITE_APP_URL}/hacker/${nick}/${currentPage}/${limit}`)
  ...
  useEffect(()=>{
    setLoading(true);
    axios.get(`${import.meta.env.VITE_APP_URL}/hackerZoneCount/${nick}`)
  ...
  return (
    <div className="w-full mt-4 rounded-[8px] p-2 bg-[#0e1726]">
      <div className='p-4'>
        ...
        <Pagination setTemp={setTemp} temp={temp} refer={`/hacker/${nick}`} page={count} active={currentPage}/>
        </>
