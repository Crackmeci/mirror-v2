...
function Archive() {
  ...
  useEffect(()=>{
    document.title = `${import.meta.env.VITE_SITE_NAME} - Archive - Page - ${currentPage}`
  },[])



  useEffect(()=>{
    if(currentPage != 1){navigate("/archive");}
    setTemp(temp+1)
  },[limit])
  ...
  useEffect(()=>{
    setLoading(true);
    Moment.locale('tr');
    axios.get(`${import.meta.env.VITE_APP_URL}/list/${currentPage}/${limit}`)
  ...
  useEffect(()=>{
    setLoading(true);
    axios.get(`${import.meta.env.VITE_APP_URL}/zoneCount`)
  ...
  return (
    <div className="w-full mt-4 rounded-[8px] p-2 bg-[#0e1726]">
      <div className='p-4'>
        ...
        <Pagination setTemp={setTemp} temp={temp} refer={`/archive`} page={count} active={currentPage}/>
        </>
